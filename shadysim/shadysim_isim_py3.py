...
# March 2022, Chris Hicks: Convert to Python 3
...
class AppLoaderCommands(object):
	...
	def send_wrapped_apdu_internal(self, data, tar, msl, kic_index, kid_index):
		...
		if (len_sig == 8):		
			...
			envelopeData = part_cnt + binascii.b2a_hex(ciph[len(ciph) - 8:]).decode('utf-8') + data
		...
		if ((spi_1 & 0x04) != 0): # check ciphering bit
			...
			envelopeData = part_head + binascii.b2a_hex(ciph).decode('utf-8')		
		else:
	...
	def load_aid_raw(self, aid, executable, codeSize, volatileDataSize = 0, nonvolatileDataSize = 0):
		...
		while len(loadData):
			...
			if len(loadData) > blockSize:
				size = blockSize // 2
				# APDU "LOAD" i.e. INS: E8, P1 = 0 unless last block, P2 = block number, Lc, data, Le = '00'
